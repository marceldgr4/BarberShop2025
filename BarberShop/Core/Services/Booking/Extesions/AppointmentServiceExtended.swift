...
extension AppointmentService {
    ...
    func fetchFilteredAppointments(filter: AppointmentDetail.AppointmentFilter) async throws
        -> [AppointmentDetail]
    {
        let allAppointments = try await fetchUserAppointments()
        ...
        case .upcoming:
            return allAppointments.filter { appointment in
                ...
                return date >= Date()
                    && ["pending", "confirmed"].contains(appointment.statusName.lowercased())
            }
